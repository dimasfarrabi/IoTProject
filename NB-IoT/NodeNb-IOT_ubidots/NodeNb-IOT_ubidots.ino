...
//--------------------------------------include library and defines VL53L0X
#include <VL53L0X.h>
VL53L0X sensor;
...
//-----------------------------------------ubidots token
String token = "BBFF-bpBY6BulIqoCryauJuoiXYDrGLLVmC";       
...
void loop() 
{
 ...
 if ((bucketPositionA==false)&&(digitalRead(RainPin)==LOW)){  //read Tipping Bucket
  ...
  if(mnt == 11|mnt == 21||mnt == 31||mnt == 41||mnt == 51||mnt == 1)
  {
    dailyRain = 0.0;
  }
}
...
void SendTo()
{
  ...
  int distance =sensor.readRangeContinuousMillimeters();  //read VL53L0X
  ...
  delay(100);
  ShowSerialData();
  mySerial.print("POST /api/v1.6/devices/misred HTTP/1.1\r\n");  //HTTP POST to file target
  delay(100);
  ShowSerialData();
