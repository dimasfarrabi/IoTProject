...
daftarMenu bahan_minum[] = {
  {"10 KG bubuk milo            ", false},
  {"5 KG jeruk peras             ", false},
  {"10 Pack teh                 ", false},
  {"20 KG gula                   ", false},
  {"10 Galon air               ", false},
  {"10 Bungkus es              ", false},
...
daftarMenu bahan_alat[] = {
  {"5Pack sedotan         ", false},
  {"20Pack gelas plastik   ", false},
  {"20Pack tutup gelas     ", false},
};
...
void kirimSMS() {
  ...
  gprsSerial.println("AT+CMGS=\"082133510659\"\r\n");
  delay(1000);
...
void loop(){
  ...
  if(nilai1 == 1){ //jika tombol1 akan masuk ke flow meter mode input 
  ...
  if(nilai3 == 1){
    ...
    if (digitalRead(pinFlow_3)==HIGH){
        ...
        lcd.setCursor(0,0);
        lcd.print("kirim data");
        payload="&item_id=3&qty=1&jml_uang=3000&user_id=2";
        kirimInternet(); 
        execute = true;
        saya_reset();
        }
